...
def LSTMModel(time_step=1,output_time_step=1,input_size=3,output_size=2,lr=0.001,width=10,depth=1):
	input = Input(batch_shape=(None,time_step,input_size))
	if depth==1: #Checking for depth to set return_sequences Flag
		x=LSTM(width,activation="relu",return_sequences=False)(input)

	elif depth>1:
		x=LSTM(width,activation="relu",return_sequences=True)(input)
		for i in range(depth-2):
			x=LSTM(width,activation="relu",return_sequences=True)(x)

		x=LSTM(width,activation="relu",return_sequences=False)(x) #Last Layer needs to have return_sequences flag as False
