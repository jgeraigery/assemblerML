...
for i in range(len(X)):
	...
	movingvalue[9,:]=X[i]		
	new=np.asarray(list(movingvalue))
	x.append(new)

x=np.asarray(x)
model.fit(x.reshape(x.shape[0],10,3),y.reshape(y.shape[0],2),epochs=20)
adam=keras.optimizers.Adam(lr=0.00001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
model.compile(loss="mse", optimizer=adam, metrics=['accuracy'])

movingvalue=np.zeros((1,10,3))
for i in np.arange(0, 50000):
    ...
    out=model.predict(movingvalue)
    model.fit(movingvalue,outBar,epochs=1)
