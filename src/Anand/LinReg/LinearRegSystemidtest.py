...
X=np.load("X10000.npy")
y=np.load("y10000.npy")

model.fit(X.reshape(10000,3),y.reshape(10000,2))
...
plt.plot( result[0], result[2],c="r",linewidth="2",label="True")
plt.ylabel("State 0")
plt.subplot(2, 1, 2)
plt.plot(result[0], result[3],c="k",linewidth="4",label="Pred")
plt.plot(result[0], result[4],c="r",linewidth="2",label="True")
plt.ylabel("State 1")
plt.xlabel("Time")
plt.legend()
plt.show()
plt.savefig("LinearRegressionSystemidtest.svg")

print "Velocity Error:",np.sqrt(np.mean((result[1]-result[2])**2))
print "Acceleration Error:",np.sqrt(np.mean((result[3]-result[4])**2))

np.save("LinearRegresult.npy",result)
print m.b,m.J
