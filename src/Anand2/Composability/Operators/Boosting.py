...
def BoostingModel(model1,model2,lr=0.001,time_step=1,input_size=3,output_size=2): 	#If Boosting a Booster Model1 must be the  Boosted Model OtherWise errors
	...
	input2=Lambda(lambda xin :K.reshape(xin,(-1,time_step*input_size)))(input) 	#Reshaping Input for Dense Networks
	action2=Lambda(lambda xin :K.reshape(xin,(-1,time_step*1)))(action) 		#Reshaping Action for Dense Networks

											#Getting Model1 Prediction

	if type(model1.output_shape) is list: 						#Finding if Model1 is Boosted Model
		if len(model1.input_shape)==3:						#Finding if Model1 is RNN
	...
	if len(model2.input_shape)==3:
		...
		prediction=Lambda(lambda xin :K.reshape(xin,(-1,time_step*output_size)))(output_a)	#Reshaping Model1 Prediction to fit Dense
		input_b=keras.layers.concatenate([prediction,action2])
		output_b = model2(input_b)
