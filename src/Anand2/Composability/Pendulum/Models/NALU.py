...
def NALUModel(input_size=3,time_step=1,output_time_step=1,output_size=2,lr=0.0001,width=10,depth=1):
	input = Input(batch_shape=(None,time_step,input_size))
	inputnew=Lambda(lambda xin :K.reshape(xin,(-1,time_step*input_size)))(input)
	#x = NALU(width,activation="relu",use_gating=True)(inputnew)
	x=BatchNormalization()(inputnew)
	x = Dense(width,activation="relu",use_bias=False)(x)

	for i in range(depth-1):
		#x = NALU(width,activation="relu",use_gating=True)(x)
		x = Dense(width,activation="relu",use_bias=False)(x)

	output = NALU(output_time_step*output_size,activation="linear",use_gating=True)(x)
