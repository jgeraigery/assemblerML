...
for i in range(time_step,len(X)-time_step):
	...
	#pred[i-time_step,:,:]=np.array(np.reshape(Xp[i:i+output_time_step,:,0:3]-X[i-1:i+output_time_step-1,:,0:3],(1,output_time_step,output_size)))
...
if options.ensemble_network=="Linear":
	...
	model = NALUModel(time_step=time_step,output_time_step=output_time_step,input_size=input_size,output_size=output_size,lr=0.001)
...
# Setting CallBacks
reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.5, patience=100, min_lr=0.00001,verbose=1)
early_stop=EarlyStopping(monitor='loss', min_delta=0.000005, patience=500, verbose=1)
...
if options.ensemble_network is not None:
	...
	out=(0.2*pred+0.8*model.predict(X))
