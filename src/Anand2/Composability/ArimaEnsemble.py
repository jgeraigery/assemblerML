...
dT = .001
m.setTimeStep(dT)
model1 = ArimaModel(time_step=3)
model2 = ArimaModel(time_step=3)

model= EnsembleModel(model1,model2,time_step=3)
...
for i in np.arange(0, 50000):
	...
	adam=keras.optimizers.Adam(lr=0.00001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
    	...
    	#model.fit(movingvalue2.reshape(1,3,3),(outBar-stateTensor[:,0:2]).reshape(1,1,2),epochs=1)
    else:
	continue
...
evaluate(result,"Images/ArimaEnsemble")
