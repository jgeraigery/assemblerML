...
from keras.layers import Convolution2D, Dense, Input, MaxPooling2D, Dropout, Flatten, ZeroPadding2D, Activation,LSTM,Bidirectional,Convolution1D,MaxPooling1D,Conv1D,SimpleRNN,Lambda,Reshape,BatchNormalization,TimeDistributed
...
def BoostingModel(model1,model2,lr=0.001,time_step=1,input_size=3,output_size=2): 	#If Boosting a Booster Model1 must be the  Boosted Model OtherWise errors
	...
	output=keras.layers.add([output_a,output_b])					#New Boosting Technique
	#output=TimeDistributed(Dense(output_size))(output)
	

	model = Model(inputs=input, outputs=output)					#Compiling Models together
