...
def SSModel(input_size=3,time_step=1,output_time_step=1,output_size=2,lr=0.001,output_function="linear"):
	input = Input(batch_shape=(None,time_step,input_size))
	inputnew=Lambda(lambda xin :K.reshape(xin,(-1,time_step*input_size)))(input)

	output = Dense(output_time_step*output_size,activation=output_function,use_bias=False)(inputnew)
