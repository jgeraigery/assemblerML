...
model1 = DenseModel(time_step=time_step,output_time_step=output_time_step,input_size=7,depth=3,output_size=1,output_function='sigmoid')
model2 = DenseModel(time_step=time_step,output_time_step=output_time_step,input_size=4,depth=3,width=50,output_size=3)
...
model,modeltrue=CyclicControlModel(model1,model2,model3,time_step=time_step,input_size=3)
print (model.summary())
...
ref=np.repeat(ref,10,axis=1)
print(ref.shape)
controlInput=np.zeros((1,10,1))
controlInput[:]=m.action_space.sample()
...
for i in np.arange(0,1000):
	if i%100==0:
		...
		stateTensor=stateTensor.reshape(1,1,3)
		stateNew=np.array(stateTensor)
		moving_state_input=np.zeros((1,time_step,3))
	...
	print ("Reference State,True State,Predicted State,Control Action,Reward,Loop Count",ref[:,0,:],stateTensor,stateNew[:,0,:],controlInput[:,0,:],reward,i)
	time.sleep(0.1)
	...
	controlInput,stateTensor=modeltrue.predict([moving_state_input,ref,controlInput])



	model.fit([moving_state_input,ref,controlInput],[ref[:,:,:]],epochs=100,verbose=0)
	stateNew=np.array(moving_state_input)	
	stateTensor,reward,_,_=m.step(controlInput[0][0])
	rewardsum.append(reward)
	stateTensor=stateTensor.reshape(1,1,3)
	if time_step>1: #To shift Inputs to the left
		moving_state_input[:,0:time_step-1,:]=moving_state_input[:,1:time_step,:]
		controlInput[:,0:time_step-1,:]=controlInput[:,1:time_step,:]

	moving_state_input[:,time_step-1,:]=np.array(stateTensor)
	controlInput[:,time_step-1,:]=np.array(controlInput[0][0][0])
