...
def custom_loss(output1,output2):
	def custom_loss(y_true, y_pred):
		#return (1*(y_true-y_pred)**2)
		return 1*(y_true-y_pred)**2+1*(output1-output2)**2
	return custom_loss
...
def CyclicControlModel(model1,model2,model3,lr=0.0001,time_step=1,input_size=2,output_size=1): 	#Model1 Controller, Model2 System ID, Model3 Inverse of Controller
	...
	input3new=crop(1,1,time_step)(input3)
	input3new=keras.layers.concatenate([input3new,output_a],axis=1)			#Concatenating Xt with Ut+1

	input5=keras.layers.concatenate([input1,input3new],axis=2)				#Concatenating Xt with Ut+1
	...
	input1new=crop(1,time_step-1,time_step)(input1)
	output_b=keras.layers.add([input1new,output_b])					#Adding TD to Previous State to get New Position Xt+1
	#output_b2=crop(2,0,1)(output_b)							#Adding TD to Previous State to get New Position Xt+1
	#input7=keras.layers.concatenate([output_b])				#Concatenating Xt+1 and Ut+1 to predict Xt

	output_c = model3(output_b)							#Get Xt
	output_c1=crop(2,0,input_size)(output_c)
	output_c2=crop(2,input_size,input_size+1)(output_c)
	output_c3=keras.layers.concatenate([output_c1,input2,output_c2],axis=-1)
	output_d = model1(output_c3)							#Get Xt
	...
	model4.compile(loss=[custom_loss(output_a,output_d),custom_loss2], optimizer=adam, metrics=['accuracy'])
	model5.compile(loss=custom_loss(output_a,output_d), optimizer=adam, metrics=['accuracy'])
