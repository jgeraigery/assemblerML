...
def new_loss(input):
	...
	return custom_loss
def TSCyclicControlModel(model1,model2,model3,lr=0.0001,time_step=1,input_size=3,output_size=1): 	#Model1 Controller, Model2 System ID, Model3 Inverse of Controller
	...
	output_a=Lambda(lambda x: (x * 4.0)-2.0)(output_a)				#Scaling Control Output from Sigmoid to be between 2.0 and -2.0

	input11=crop(1,time_step-1,time_step)(input1)
	input5=keras.layers.concatenate([input11,output_a],axis=-1)				#Concatenating Xt with Ut+1
