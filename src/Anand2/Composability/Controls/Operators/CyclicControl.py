...
def CyclicControlModel(model1,model2,model3,lr=0.0001,time_step=1,input_size=2,output_size=1): 	Model1 Controller, Model2 System ID, Model3 Inverse of Controller
	...
	input4=keras.layers.concatenate([input1,input2,input3],-1)			#Concatenating Xt,et,Ut to predict Ut+1
	output_a= model1(input4)							#Predicting Ut+1

	input5=keras.layers.concatenate([input1,output_a],-1)				#Concatenating Xt with Ut+1
	...
	input6=keras.layers.add([input1,output_b])					#Adding TD to Previous State to get New Position Xt+1
	input7=keras.layers.concatenate([input6,output_b])				#Concatenating Xt+1 and Ut+1 to predict Xt
	output_c = model3(input_b)							#Get Xt

	model = Model(inputs=[input1,input2,input3], outputs=[output_b,output_c])	#Compiling Models together
