...
for i in np.arange(0, 100000):
    ...
    if ( np.mod(i,50) == 0 ):
        print "Loop-",i
        controlInput=0
        if (i%1000==0 and i!=0 and i>10000):
                controlInput = getControlInput()
		m.Km=1
		#m.b+=10*dT
		m.update()
        elif (i%1000==0 and i!=0 and i<10000):
                controlInput = getControlInput()
    ...
    if i<10000:
   	...
   	X2.append(np.concatenate((out,stateTensor),1))
	...
	print np.asarray(error).shape
    	out=model.predict(stateTensor)
        model2.fit(np.asarray(X2).reshape(20000-1,5),(np.asarray(error)).reshape(20000-1,2),epochs=50,batch_size=32)
	adam=keras.optimizers.Adam(lr=0.00001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
        model.compile(loss="mse", optimizer=adam, metrics=['accuracy'])
    elif i>30000:
    	out=model.predict(stateTensor)
	outerror=model2.predict(np.concatenate((out,stateTensor),1))
