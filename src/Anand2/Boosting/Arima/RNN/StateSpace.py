...
def rnnmodel(window=3,features=5):
        ...
        x=SimpleRNN(10,activation="relu",return_sequences=True,use_bias=False)(input)
        x=SimpleRNN(10,activation="relu",use_bias=False)(x)
...
for i in np.arange(0, 100000):
    ...
    if i<10000:
        out=np.zeros((1,2))
	moving[0:2,:]=moving[1:3,:]
	moving[2,:]=stateTensor
	moving2=np.asarray(list(moving))
   	X.append(moving2)
        y.append(outBar-stateTensor[:,0:2])
    elif i==10000:
        out=np.zeros((1,2))
        model.fit(np.asarray(X),(np.asarray(y)).reshape(10000,2),epochs=50,batch_size=32)
	...
	moving[2,:]=stateTensor
	moving2=np.asarray(list(moving))
    	out=model.predict(moving2.reshape(1,3,3))
