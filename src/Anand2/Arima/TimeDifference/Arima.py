...
class Motor:
    ...
    def step(self, u):
        ...
        noisedot[:,:]=np.random.normal(0.0, 0.01, 2)
        noise= np.zeros([1,2],dtype=float)
        noise[:,:]=np.random.normal(0.0, 0.001, 2)
...
for i in np.arange(0, 50000):
	...
	model = auto_arima(np.random.normal(0,1,1000), start_p=1, start_q=1,max_p=3, max_q=3, m=1,start_P=0,d=0, D=1, trace=True,seasonal=False,error_action="ignore",suppress_warnings=True,stepwise=True)
	model2 =auto_arima(np.random.normal(0,1,1000),start_p=1, start_q=1,max_p=3, max_q=3, m=1,start_P=0,d=0, D=1, trace=True,seasonal=False,error_action="ignore",suppress_warnings=True,stepwise=True)
    elif i>10000:
	#model.fit(np.asarray(y)[:,:,0].flatten())
    	out[0][0]=(model.predict(n_periods=1))
	#model2.fit(np.asarray(y)[:,:,1].flatten())
